...
tags:
  ...
  - context-rot
key_insight: >-
  Context engineering reframes LLM optimization from discrete prompt crafting to
  continuous curation of a finite 'attention budget' across multi-turn agent
  loops—builders must treat context as a scarcest resource where every token
  competes, especially as agents accumulate state that could be relevant but
  degrades performance via context rot.
relevance_scores:
  topic_relevance: 9
