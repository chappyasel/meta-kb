...
tags:
  ...
  - proactive-retrieval
key_insight: >-
  Hipocampus solves the 'unknown unknowns' problem in agent memory by
  maintaining a lightweight topic index (ROOT.md) that makes implicit context
  discoverable without search queries—achieving 21x better performance on
  cross-domain recall than unstructured memory, which matters for agents that
  need to autonomously connect decisions across weeks of conversation without
  explicit prompting.
stars: 145
forks: 11
...
**Topics:** agent-memory, ai-agent, ai-memory, claude-code, compaction-tree, context-management, knowledge-management, llm, long-term-memory, openclaw, rag, vector-search
