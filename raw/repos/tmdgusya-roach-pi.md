---
url: 'https://github.com/tmdgusya/roach-pi'
type: repo
author: tmdgusya
date: '2026-04-08'
tags:
...
key_insight: >-
  Multi-agent orchestration with autonomous subagents and parallel task dispatch
  enables the pi agent to scale from single-task execution to coordinated
  investigation and implementation—but requires explicit coordination primitives
  (ask_user_question, subagent delegation modes, phase management) to prevent
  agent hallucination and maintain control flow determinism across asynchronous
  worker boundaries.
stars: 41
forks: 5
relevance_scores:
  ...
  composite: 6.6
  reason: >-
    Demonstrates concrete multi-agent orchestration patterns (parallel subagent
    dispatch, phase management, coordination primitives) in a production
    TypeScript extension with documented architecture, making it a transferable
    reference for multi-agent coordination design even though it targets a
    coding agent rather than a general-purpose KB system.
---
## roach-pi
...
**Topics:** extensions, pi, pi-coding-agent
