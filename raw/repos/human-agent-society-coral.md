...
tags:
  ...
  - skill-registry
key_insight: >-
  CORAL's multi-agent self-evolution framework with persistent shared knowledge
  (attempts, notes, skills in `.coral/public/`) enables agents to build on each
  other's discoveries without sync overhead—critical for autoresearch systems
...
**Topics:** agent-framework, alpha-evolve, alphaevolve, autoresearch, autoresearcher, claude-code, codex, coding-agent, large-language-models, llm-agents, multi-agent, opencode, optimize, self-evolving, self-evolving-agents, self-improving
