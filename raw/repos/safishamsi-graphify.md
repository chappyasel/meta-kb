...
key_insight: >-
  Graphify transforms heterogeneous knowledge sources (code, papers, images,
  docs) into a queryable knowledge graph that can be persisted and incrementally
  updated, reducing token overhead by 71.5x per query compared to raw file
  retrieval—critical for agentic workflows where multiple agents need shared,
  always-current context across long sessions.
stars: 7021
forks: 722
relevance_scores:
  topic_relevance: 9
  ...
  composite: 8.9
  reason: >-
    Graphify is a directly on-topic tool implementing knowledge graph
    construction from heterogeneous sources as a Claude Code skill, with
    SKILL.md integration, Obsidian vault output, persistent graph querying,
    incremental updates, and 71.5x token reduction benchmarks—covering Knowledge
    Substrate, Context Engineering, and Agent Architecture pillars with a
