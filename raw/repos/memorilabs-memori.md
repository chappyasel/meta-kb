...
tags:
  ...
  - LoCoMo-benchmark
key_insight: >-
  Memori's SQL-native architecture with structured memory extraction from agent
  actions (not just text) enables precise long-context recall while dramatically
  reducing token footprint—critical for production systems where context-window
